...
import { LoginAuthProps, SignupProps , VerficationProps, ResendProps, ConfirmPasswordAPI, ForgetPasswordProps } from '../Lib/interface/Authentication';
...
class AuthService implements Auth {
   ...
   */
  private async postRequest(endpoint: string, data: object): Promise<AxiosResponse> {
    try {
   ...
   */
  async resendOTP(props: ResendProps, setTimer: Function, setMsg: Function): Promise<void> {
    try {
      // Send request to resend OTP
      const response: AxiosResponse = await this.postRequest(this.server.Auth.resend, { ...props.value });
   ...
   */
  async verifyOTP(props: VerficationProps, setMsg: Function): Promise<void> {
    try {
      // Send request to verify OTP
      const response: AxiosResponse = await this.postRequest(this.server.Auth.otpVerification, { ...props.value });
   ...
   */
  async forgotPassword(props: ForgetPasswordProps,setMsg: Function): Promise<void> {
    try {
      // Send request to initiate password reset process
    const response: AxiosResponse = await this.postRequest(this.server.Auth.forget, { EMAIL: props.EMAIL });
  ...
  async confirmPasswordChange(props: ConfirmPasswordAPI): Promise<void> {
    try {
      ...
      const response: AxiosResponse = await this.postRequest(this.server.Auth.confirmPasswordChange, { PASSWORD:props.PASSWORD });
   ...
   */
  async login(props: LoginAuthProps, setMsg: Function, navigate: Function): Promise<void> {
    try {
      // Send request to log in the user
      const response: AxiosResponse = await this.postRequest(this.server.Auth.login, { ...props.value });
  ...
  async signup(props: SignupProps): Promise<void> {
    try {
      ...
      const response: AxiosResponse = await this.postRequest(this.server.Auth.signup, { ...props.value });
  ...
  async logout(): Promise<void> {
    try {
      ...
      const response: AxiosResponse = await this.postRequest(this.server.Auth.logout, {});
